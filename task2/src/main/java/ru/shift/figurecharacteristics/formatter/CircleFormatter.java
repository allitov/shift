...
public class CircleFormatter implements FigureFormatter<Circle> {
    ...
    public String format(Circle circle) {
        return "Тип фигуры: " + circle.getName() + LINE_SEPARATOR +
                ...
                "Периметр: " + DECIMAL_FORMAT.format(circle.getPerimeter()) + " " + UNITS + LINE_SEPARATOR +
                "Радиус: " + DECIMAL_FORMAT.format(circle.getRadius()) + " " + UNITS + LINE_SEPARATOR +
                "Диаметр: " + DECIMAL_FORMAT.format(circle.getDiameter()) + " " + UNITS;
    }
}
